...
class YoujiaEvaluator(BaseEvaluator):
    ...
    def evaluate(
        ...
        DEPRECATED: This evaluator is deprecated. Use HarmBench or JailbreakBench instead.
        ...
        Args:
            ...
            responses: Dict mapping prompt_id -> jailbreak response text (pre-generated by Task)
        ...
        # Use pre-generated jailbreak responses
        jailbreak_dict = responses
        ...
        if baseline_responses:
            baseline_dict = baseline_responses
        else:
            # Generate baseline responses if not provided (legacy behavior)
            target_model_candidates = [
                p.get('target_model') for p in prompts if p.get('target_model')
            ]
            if target_model_candidates:
                logger.warning("YoujiaEvaluator: generating baseline responses internally (deprecated)")
                target_service = LLMServiceFactory.create(target_model_candidates[0])
                baseline_prompts = [
